...
extern "C" int ref2va_delta_factors(const float* A, const float* B, const float* alpha,
    float* transition, float* injection, int count, cudaStream_t stream, int device) {
  ...
  if (configured_device != device) {
    result = cudaFuncSetAttribute(vdn_delta_factors_kernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize, kXsBytes);
    if (result != cudaSuccess) return int(result);
    configured_device = device;
  }
  if (count > 0) vdn_delta_factors_kernel<<<count, kBlockSize, kXsBytes, stream>>>(
      A, B, alpha, transition, injection);
  return int(cudaGetLastError());
}
extern "C" const char* ref2va_cuda_error(int code) { return cudaGetErrorString(cudaError_t(code)); }
} // namespace vdn_delta_factors
} // namespace sglang
